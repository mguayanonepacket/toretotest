...
def establishConnection(host, username):
    ...
    try:
        ...
        ssh_client.connect(
            hostname=host, username=username, pkey=private_key, timeout=300, banner_timeout=60, auth_timeout=60
        )
...
def getArguments():
    ...
    all_args.add_argument(
        ...
        nargs="?",
        required=False,
        help="Include TXT format diff (by default is deactivated)",
    )
    all_args.add_argument(
        "-no-html",
        "--NoHtmlDiff",
        ...
        nargs="?",
        required=False,
        help="Include HTML format diff (by default is activated)",
    )
    ...
    racks = []
    devices = []
    excludeHtmlDiff = False
    includeTxtDiff = False
    ...
    if args["txtDiff"]:
        includeTxtDiff = args["txtDiff"]
    if args["NoHtmlDiff"]:
        excludeHtmlDiff = args["NoHtmlDiff"]

    return option, username, intermediateRun, args, sites, devs, pods, racks, devices, includeTxtDiff, excludeHtmlDiff
...
def genHTMLDiffFile(file_pre, file_post, command, host, deviceType, intermediateArg):
    ...
    commandName = removeInvalidChars(command)
    ...
    if output_file:
        if deviceType == "arista_eos":
            html_diff = LimitedRecursionHtmlDiff(recursion_limit=12)
...
def removeInvalidChars(string):
    ...
    # Define a regular expression pattern to match invalid characters and whitespace
    invalid_chars_pattern = r'[\/:*?"<>|^ ]'

    # Remove invalid characters and whitespace from the file name using re.sub()
    string = re.sub(invalid_chars_pattern, "", string)

    return string


def childPreChecks(host, deviceType, username, option, intermediateArg, includeTxtDiff, excludeHtmlDiff):
    ...
    with open(commandFileName) as file:  # GET COMMANDS AND OUTPUTS
        for command in file:
            ...
            filename = removeInvalidChars(command)
            ...
            try:
                ...
                output_lines = stdout.readlines()  # Read lines from stdout
                output = "".join(output_lines)  # Combine lines into a single string
...
def getCMDOutput(option, intermediateArg, host, command, deviceType, includeTxtDiff, excludeHtmlDiff):
    ...
    if option == "CurrentStateComparison":
        ...
        filenameTMP = (
            "OUTPUT/" + intermediateDir + "/" + filename + "-intermediate-" + intermediateArg + "-" + host + ".txt"
        )
    else:
        if not os.path.exists("OUTPUT/TMP-POST-/"):
            try:
    ...
    # Closing postchecks output file
    f.close()
...
def combineTXTFiles(host, intermediateArg, fileType):
    ...
    if option == "CurrentStateComparison":
        ...
        if fileType == "post":
            postDir = "intermediate-" + intermediateArg + "-post-outputs"
            if not os.path.exists("OUTPUT/" + str(postDir)):
                try:
                    os.mkdir("OUTPUT/" + str(postDir))
                except:
                    print("The OUTPUT directory already exists")
                    pass
        ...
        try:
            if fileType == "diff":
                ...
                cmd = [
                    ...
                    f"cat OUTPUT/TMP-{intermediateDir}/*{host}*-diff.txt >> {output_file}",
                ]
            elif fileType == "post":
                cmd = [
                    "sh",  # Run the command through a shell
...
def combineHTMLFiles(host, intermediateArg):
    ...
    if intermediateArg:
        ...
        output_file = "HTML/" + intermediateDir + "/" + host + "-pre-intermediate-" + intermediateArg + "-diff.html"
        try:
            cmd = [
                "sh",  # Run the command through a shell
                "-c",  # Use a shell command
...
def childPostChecks(host, deviceType, username, option, intermediateArg, includeTxtDiff, excludeHtmlDiff):

    print("Thread created for", host)

    # Number of workers (Don't set it more than 15 for JUniper devices')
    MAX_WORKERS = 20 if deviceType != "juniper_junos" else 10
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        with open(
            commandFileName
        ) as file:  # GET COMMANDS AND OUPUTS AND COMPARE THEM WITH THE PREVIOUS OUTPUTS AND STORED IN THE TEMP DIRECTORY
            for command in file:
                command = command.strip()
                thread = executor.submit(
                    getCMDOutput, option, intermediateArg, host, command, deviceType, includeTxtDiff, excludeHtmlDiff
                )
                threads.append(thread)
                if (
                    deviceType == "juniper_junos"
                ):  # This is to avoid the rate limit of concurrent connections of juniper devices (20/min)
                    time.sleep(4)

    # Wait for all threads to complete
    for thread in threads:
        thread.result()  # This will re-raise any exception that occurred in the thread

    # TXT Diff
    if includeTxtDiff:
        combineTXTFiles(host, intermediateArg, "diff")
    ...
    # Post Ouputs
    combineTXTFiles(host, intermediateArg, "post")
...
def createThreads(hosts4Threads, username, option, intermediateRun, result_queue, includeTxtDiff, excludeHtmlDiff):
    with ThreadPoolExecutor(max_workers=20) as executor:  # CREATE THREADS FOR EACH DEVICE
        futures = []
        for host in hosts4Threads:
            ...
            futures.append(
                executor.submit(
                    ...
                    includeTxtDiff=includeTxtDiff,
                    excludeHtmlDiff=excludeHtmlDiff,
                )
            )
...
if __name__ == "__main__":
    ...
    # Get arguments
    option, username, intermediateRun, args, sites, devs, pods, racks, devices, includeTxtDiff, excludeHtmlDiff = (
        getArguments()
    )
    ...
    if option == "pre":
        ...
        if numProcesses > len(hosts4Threads):
            numProcesses = len(hosts4Threads)

        for process_num, devices_list in enumerate(hosts4Threads):
            process = multiprocessing.Process(
                target=createThreads,
                args=(devices_list, username, option, intermediateRun, result_queue, includeTxtDiff, excludeHtmlDiff),
            )
